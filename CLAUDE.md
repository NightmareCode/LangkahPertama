...
|---|---|---|---|
| `/` | Studio main page | EN | **Cinematic "First Light → Descent"** (redesigned 2026-06-21 toward a luxury video reference). Dark + gold core, Space Grotesk/Inter/JetBrains Mono + **Anton** (kinetic display) + **Cormorant Garamond** (serif). A fixed `.skyveil` value-key fades a warm DAWN hero into the dark body via `--hf` (one rAF "cinema" loop sets `--hf`/`--descent`, toggles `nav.over-light`, and parallaxes `[data-speed]` elements by their LIVE position — only the absolutely-positioned `.kinetic-word` "CRAFT" drifts; flow text must NOT use data-speed). Hero = Three.js gold sun-core (assets/hero.glb: HeroRig/RingA/RingB/RingC/SatRig — keep those node names; drag/tap/pulse) + big Anton "first step" (deep gold, readable on cream). The page ALTERNATES light/dark: light hero → dark services/work → **LIGHT `.wordmark-moment`** ("Langkah Pertama") → dark process → **LIGHT `#about.chapter-light`** ("I'm Far" paper chapter) → dark close. Reveals use `.reveal` (opacity) + `.reveal-clip` (opacity+rise+blur — NEVER clip-path: inset that zeros the box, it deadlocks the IO). Closing `.descent-end` = echoed "Take the first step." Contrast rule: on the light hero/chapters keep DARK ink text + DEEP gold (light gold on cream is unreadable). To SEE WebGL/scroll states (preview screenshots time out), use the local Playwright harness in `.pw/` (gitignored). |
