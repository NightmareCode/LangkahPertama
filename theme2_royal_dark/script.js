document.addEventListener('DOMContentLoaded', () => {
    ...
    if (isPreview) {
        ...
        mainContent.classList.remove('hidden');
        setTimeout(() => {
            const elementsToAnimate = document.querySelectorAll('.slide-in-left, .slide-in-right, .slide-in-bottom');
    ...
    const observer = new IntersectionObserver((entries, obs) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                // For this theme, once visible, do not unobserve so they can be triggered again? 
                // Wait, it's usually better to unobserve so it doesn't repeatedly animate and hurt performance on mobile
